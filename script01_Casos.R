...
write.csv2(x = df, file = "Banco_original/03_df_paraquat_ajustado.csv")
...
df %>% 
  ...
  summarise(N = n()) %>% 
  #na.omit() %>% 
  arrange(desc(N)) %>% kable() |> kable_classic()
# Estado
df %>% 
  group_by(SG_UF) %>% 
  summarise(N = n()) %>% 
  #na.omit() %>% 
  arrange(desc(N)) %>% print(n = 28) %>% kable() |> kable_classic()
...
df %>% 
  ...
  summarise(N = n()) %>% 
  #na.omit() %>% 
  arrange(desc(N)) %>% head(20) |>  kable() |> kable_classic()
