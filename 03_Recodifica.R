...
df <- readRDS("Dados_processados/02_df_concat_filtrado.rds")
...
df <- df %>%
  mutate(
    FAIXA_ETARIA = cut(
      ...
      labels = etaria_sinan,                  # Vetor de rótulos (nomes das faixas)
      include.lowest = TRUE,                   # Inclui o limite inferior (0 ano)
      right = FALSE                            # Define o intervalo como [início, fim), ou seja, o limite superior é exclusivo
    )
  )
...
# RDS
saveRDS(object = df_final,
        file = "Dados_processados/03_df_paraquat.rds")

# XLSX
write.xlsx(x = df_final,
           file = "Dados_processados/03_df_paraquat.xlsx")
